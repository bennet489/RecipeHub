...
class TestUserModel(unittest.TestCase):
    ...
    def test_follow(self):
        ...
        self.assertEqual(user1_following[0].username, 'lizzy')
        self.assertEqual(user2_followers[0].username, 'john')

        user1.unfollow(user2)
        db.session.commit()
        self.assertFalse(user1.is_following(user2))
        self.assertEqual(user1.following(), 0)
        self.assertEqual(user2.follower_count(), 0)
    ...
    def test_follow_post(self):
        ...
        post1 = Post(title='how to make pizza', body="Post from lizzy",
                     author=user1, timestamp=now + timedelta(seconds=1))
        post2 = Post(title='how to make yogurt', body='post from john',
                     author=user2, timestamp=now + timedelta(seconds=4))
        post3 = Post(title='kontomire stew recipe', body='post from Joe',
                     author=user3, timestamp=now + timedelta(seconds=2))
        post4 = Post(title='coconut rice recipe', body='Post by Kate',
